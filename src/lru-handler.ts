...
interface LRUCacheOptions<T> {
	maxSize?: number;
	maxWeight?: number;
	calculateItemWeight?: (value: T) => number;
	ttl?: number;
}
...
class LRUCache<T> {
	...
	private calculateItemWeight: ((value: T) => number) | null;
	private isWeightedMode: boolean;
	private ttl: number;
	private hits: number;
	...
	static generateKey(messages: Message[]): string {
	...
	private _moveToFront(node: CacheNode<T>): void {
		if (node === this.head) {
			// Already at front
			return;
		}
	...
	private _addToFront(node: CacheNode<T>): void {
		node.next = this.head;
		node.prev = null;
	...
	set(key: string, value: T): void {
		...
		let itemWeight =
			this.isWeightedMode && this.calculateItemWeight ? this.calculateItemWeight(value) : 0;
		...
		const newNode: CacheNode<T> = {
			...
			value,
			weight: itemWeight,
			timestamp: this.ttl > 0 ? Date.now() : undefined,
			next: null,
			prev: null,
		};
	...
	private _makeSpaceForWeight(requiredWeight: number): void {
