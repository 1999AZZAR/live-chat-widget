export function generateWidgetHTML(url: URL): string {
	// Get URL parameters
	...
	const colorDefaults: Record<string, any> = {
		light: {
			accentColor: '#6200EE',
	...
	const styles = {
		...
		inputBorderRadius: params.get('border-radius')
			? `calc(${params.get('border-radius')} + 6px)`
			: '24px',
	};
