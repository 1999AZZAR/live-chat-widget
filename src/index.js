...
async function getSystemPrompt(env) {
    ...
    if (env && env.SYSTEM_PROMPT) {
        try {
            ...
            if (kvPrompt) {
                ...
                if (env.DEBUG)
                    console.log('Successfully loaded persona from SYSTEM_PROMPT KV');
            }
        }
        catch (error) {
    ...
    const crawlLinks = crawlFileContent
        .split('\n')
...
function deduplicateResponse(text, env) {
    if (!text)
        return text;
    ...
    if (sentences && sentences.length > 3) {
        ...
        for (const sentence of sentences) {
            ...
            for (const seen of Array.from(seenSentences)) {
            ...
            try {
                ...
                if (clientIP !== 'unknown_ip') {
                    ...
                    if (ipRequestCount >= 30)
                        return { exceeded: true, retryAfter: Math.ceil(secondsUntilNextMinute) };
                }
